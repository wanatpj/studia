 ...
 *				do_profile				     *
 *===========================================================================*/
PUBLIC int do_profile()
{
  static int obs = 0;
  int res = (OK);
  if(obs != 0 && (obs != who||m_in.turn_off == 1))
  	return (EPERM);
  if(m_in.turn_off==1){
  	res = sys_profile_stop(m_in.profiling_outcome, proc_from_pid(who));
  } else {
  	res = sys_profile_start(proc_from_pid(m_in.profiling_track_pid));
  }
  obs = who;
  return(OK);
}
